        ...
        class ThreeJSPlotter {
            ...
            updateAxesAndGrid() {
                ...
                this.plotGroup.children.forEach(mesh => {
                    const objectBounds = this.calculateObjectBounds(mesh);
                    if (!objectBounds.isEmpty()) {
                        combinedBounds.union(objectBounds);
                        hasContent = true;
                    }
                });
                if (hasContent && !combinedBounds.isEmpty()) {
                    bounds = combinedBounds.expandByScalar(5); 
                } else {
                    bounds = new THREE.Box3(new THREE.Vector3(-20, -20, -20), new THREE.Vector3(20, 20, 20));
                }
                if (!hasContent && this.app.currentMode === 'playground') {
                    this.renderDirty = true;
                    return;
                }
        ...
        class AppController {
            constructor() {
                ...
                this.tutorialSteps = [
                    {
                        title: "Welcome to 6Axis Animation !",
                        text: "6Axis Animation a 3D Grapher and Animator, Let's take a quick tour of this app's main features to get you started."
                    },
                    {
            ...
            init() {
                ...
                this.colormaps = {
                    ...
                    Reds: { name: 'Reds', gradient: 'linear-gradient(90deg, #fee0d2, #de2d26)' },
                ...
                if (!localStorage.getItem('plotterProTutorialCompleted')) {
                    setTimeout(() => {
                        this.startTutorial();
                    }, 500); 
                }
            ...
            setupEventListeners() {
                ...
                document.getElementById('fullscreen-toggle').addEventListener('click', () => { this.appGrid.classList.toggle('fullscreen-zen-mode'); setTimeout(() => this.plotter.onResize(), 310); });
            ...
            initializePresets() {
                this.presets = {
                    surface: [
                        { name: 'Pulsing Wave', equation: '5*\\sin(0.5*\\sqrt{x^2+y^2} - T)' },
                        { name: 'Gaussian Pulse', equation: '8*e^(-(x^2+y^2)/(T+0.1))' },
                        { name: 'Warping Saddle', equation: '0.1*(x^2 - y^2) * \\cos(T)' },
                        { name: 'Monkey Saddle', equation: '0.1*(x^3 - 3*x*y^2)' },
                        { name: 'Ripple', equation: '2*\\cos(x + T)*\\sin(y + T)' },
                        { name: 'Funnel', equation: '5/\\sqrt{x^2+y^2+1}' },
                    ],
                    parametric: [
                        { name: 'Pulsing Sphere', xExpr: '(5+2*\\sin(T))\\cos(u)\\cos(v)', yExpr: '(5+2*\\sin(T))\\cos(u)\\sin(v)', zExpr: '(5+2*\\sin(T))\\sin(u)', uMin: -1.57, uMax: 1.57, vMax: 6.28 },
                        { name: 'Wavy Torus', xExpr: '(5+2*\\cos(u))\\cos(v)', yExpr: '(5+2*\\cos(u))\\sin(v)', zExpr: '2*\\sin(u) + \\sin(v*5 + T)', uMax: 6.28, vMax: 6.28 },
                        { name: 'Seashell', xExpr: 'u/2*(1-u/(2*pi))*\\cos(u)* (1+\\cos(v))', yExpr: 'u/2*(1-u/(2*pi))*\\sin(u)*(1+\\cos(v))', zExpr: 'u/2*(1-u/(2*pi))*\\sin(v) + 2*u/ (2*pi) + T/2', uMin: 0, uMax: 6.28, vMin: 0, vMax: 6.28, quality: 150 },
                        { name: 'Möbius Strip', xExpr: '(4+v*\\cos(u/2))\\cos(u)', yExpr: '(4+v*\\cos(u/2))\\sin(u)', zExpr: 'v*\\sin(u/2)', uMax: 6.28, vMin: -1, vMax: 1, quality: 100 },
                        { name: 'Klein Bottle', xExpr: '(2+\\cos(u/2)*\\sin(v)-\\sin(u/2)*\\sin(2*v))*\\cos(u)', yExpr: '(2+\\cos(u/2)*\\sin(v)-\\sin(u/2)*\\sin(2*v))*\\sin(u)', zExpr: '\\sin(u/2)*\\sin(v)+\\cos(u/2)*\\sin(2*v)', uMax: 6.28, vMax: 6.28, quality: 150 },
                        { name: 'Helicoid', xExpr: 'u*\\cos(v)', yExpr: 'u*\\sin(v)', zExpr: '1.5*v', uMin: -5, uMax: 5, vMin: -10, vMax: 10, quality: 80 },
                    ],
                    curve: [
                        { name: 'Wobble Helix', xExpr: '(3+\\sin(T)) * \\cos(t)', yExpr: '(3+\\sin(T)) * \\sin(t)', zExpr: '0.5*t', tMax: 30 },
                        { name: 'Trefoil Knot', xExpr: '\\sin(t)+2*\\sin(2*t)', yExpr: '\\cos(t)-2*\\cos(2*t)', zExpr: '-\\sin(3*t)', tMax: 6.28 },
                        { name: 'Lissajous 3D', xExpr: '4*\\sin(3*t + T/2)', yExpr: '4*\\cos(5*t)', zExpr: '4*\\sin(4*t)', tMax: 6.28, quality: 1000 },
                        { name: 'Butterfly 3D', xExpr: '\\sin(t)*(e^\\cos(t)-2*\\cos(4*t)-(\\sin(t/12))^5)', yExpr: '\\cos(t)*(e^\\cos(t)-2*\\cos(4*t)-(\\sin(t/12))^5)', zExpr: '2*\\sin(T/5)', tMin: 0, tMax: 12.56, quality: 1000 },
                        { name: 'Spiral Sphere', xExpr: '5*\\cos(t)*\\sin(t*0.2)', yExpr: '5*\\sin(t)*\\sin(t*0.2)', zExpr: '5*\\cos(t*0.2)', tMax: 62.8, quality: 1500 },
                    ],
                    vector: [
                        { name: 'Rotational Flow', fx: '-y*\\cos(T)', fy: 'x*\\cos(T)', fz: 'sin(T)' },
                        { name: 'Pulsing Source', fx: 'x*\\cos(T)', fy: 'y*\\cos(T)', fz: 'z*\\cos(T)' },
                        { name: 'Magnetic Field', fx: 'z', fy: '0', fz: '-x', zMin: -1, zMax: 1 },
                        { name: 'Vortex', fx: '-y', fy: 'x', fz: '0' },
                        { name: 'Saddle Flow', fx: 'x', fy: '-y', fz: '0' },
                    ],
                    implicit: [
                        { name: 'Sphere', equation: 'x^2+y^2+z^2-16' },
                        { name: 'Torus', equation: '(x^2+y^2+z^2+12)^2 - 64*(x^2+y^2)' },
                        { name: 'Gyroid', equation: '\\cos(x)*\\sin(y)+\\cos(y)*\\sin(z)+\\cos(z)*\\sin(x)' },
                        { name: 'Heart', equation: '(x^2 + (9/4)*y^2 + z^2 - 1)^3 - x^2*z^3 - (9/80)*y^2*z^3', quality: 80 },
                        { name: 'Dini\'s Surface', equation: '((x^2+y^2)*\\cos(T) - z^2*\\sin(T))^2 - 4*z^2*(1-x^2-y^2)', quality: 80 }
                    ],
                    'single-vector': [
                 ...
                 this.playgroundPresets = {
                    'Atom': [
                        { type: 'parametric', name: 'Nucleus', config: { xExpr: '1.5*\\cos(u)*\\cos(v)', yExpr: '1.5*\\cos(u)*\\sin(v)', zExpr: '1.5*\\sin(u)', uMin: -1.57, uMax: 1.57, vMax: 6.28, quality: 40 }, colormap: 'Reds' },
                        { type: 'curve', name: 'Electron 1', colormap: 'Plasma', config: { xExpr: '10*\\cos(t)', yExpr: '10*\\sin(t)', zExpr: '0', tMax: 6.28 }, animation: { modes: {v: false, p: true}, T: {max: 6.28}, speed: 1, pos: {x:'0', y:'0', z:'0'}, rot: {x:'20', y:'20', z:'T*57.3'}, scale: {x:'1',y:'1',z:'1'}} },
                        { type: 'curve', name: 'Electron 2', colormap: 'Viridis', config: { xExpr: '10*\\cos(t)', yExpr: '10*\\sin(t)', zExpr: '0', tMax: 6.28 }, animation: { modes: {v: false, p: true}, T: {max: 6.28}, speed: 0.7, pos: {x:'0', y:'0', z:'0'}, rot: {x:'90', y:'T*57.3', z:'0'}, scale: {x:'1',y:'1',z:'1'}} },
                        { type: 'curve', name: 'Electron 3', colormap: 'Jet', config: { xExpr: '10*\\cos(t)', yExpr: '10*\\sin(t)', zExpr: '0', tMax: 6.28 }, animation: { modes: {v: false, p: true}, T: {max: 6.28}, speed: 1.2, pos: {x:'0', y:'0', z:'0'}, rot: {x:'-20', y:'-20', z:'-T*57.3'}, scale: {x:'1',y:'1',z:'1'}} }
                    ],
                    'Rocket Launch': [
                        { type: 'parametric', name: 'Rocket', config: { xExpr: '0.5*cos(v)', yExpr: '0.5*sin(v)', zExpr: 'u', uMin: 0, uMax: 4, vMax: 6.28, quality: 20 }, colormap: 'Reds', animation: { modes: {v:false, p:false, physics:true}, T: { min: 0, max: 20 }, speed: 1.0, path: { waypoints: [{ x: 0, y: 0, z: 0 }, { x: 5, y: 20, z: 50 }], autoRotate: true, physics: { mass: 10, thrust: 'if(T < 10, 2500, 0)', gravity: 9.8, drag: 0.5 } } } }
                    ],
                    'Rocket Parts': [
                        { type: 'parametric', name: 'Body', config: { xExpr: '2*\\cos(v)', yExpr: '2*\\sin(v)', zExpr: 'u', uMin: 0, uMax: 15, vMax: 6.28, quality: 50 }, colormap: 'Greys' },
                        { type: 'parametric', name: 'Nose Cone', config: { xExpr: '(2-u)*\\cos(v)', yExpr: '(2-u)*\\sin(v)', zExpr: '15+u*1.5', uMin: 0, uMax: 2, vMax: 6.28, quality: 50 }, colormap: 'Reds' },
                        { type: 'vector', name: 'Exhaust', config: { fx: '0.2*\\sin(T*2)', fy: '0.2*\\cos(T*2)', fz: '-10*e^(-(x^2+y^2))', xMin: -2, xMax: 2, yMin: -2, yMax: 2, zMin: -5, zMax: 0, density: 15 }, colormap: 'Hot' },
                    ],
                    'Saturn': [
                        { type: 'parametric', name: 'Planet', config: { xExpr: '4*\\cos(u)*\\cos(v)', yExpr: '4*\\cos(u)*\\sin(v)', zExpr: '4*\\sin(u)', uMin: -1.57, uMax: 1.57, vMax: 6.28, quality: 80 }, colormap: 'Plasma', animation: { modes: {v:false, p:true}, T: {max: 6.28}, speed: 0.2, rot: {z:'T*57.3'} } },
                        { type: 'parametric', name: 'Rings', config: { xExpr: '(6+u)*\\cos(v)', yExpr: '(6+u)*\\sin(v)', zExpr: '0.1*\\sin(v*20)', uMin: 0, uMax: 3, vMax: 6.28, quality: 100 }, colormap: 'Greys', animation: { modes: {v:false, p:true}, T: {max: 6.28}, speed: 0.2, rot: {x:'10'} } }
                    ],
                    'Ceiling Fan': [
                        { type: 'parametric', name: 'Motor', config: { xExpr: '0.5*\\cos(v)', yExpr: '0.5*\\sin(v)', zExpr: 'u', uMin: -0.5, uMax: 0.5, vMax: 6.28, quality: 20 }, colormap: 'Greys' },
                        { type: 'parametric', name: 'Blade 1', config: { xExpr: '(1+u)*\\cos(v)', yExpr: '(1+u)*\\sin(v)', zExpr: '0.2*u*\\cos(v)', uMin: 0, uMax: 4, vMin: 0, vMax: 0.5, quality: 20 }, colormap: 'Jet', animation: { modes: {v:false, p:true}, T: {max: 6.28}, speed: 2, rot: {z:'T*57.3'} } },
                        { type: 'parametric', name: 'Blade 2', config: { xExpr: '(1+u)*\\cos(v)', yExpr: '(1+u)*\\sin(v)', zExpr: '0.2*u*\\cos(v)', uMin: 0, uMax: 4, vMin: 0, vMax: 0.5, quality: 20 }, colormap: 'Jet', animation: { modes: {v:false, p:true}, T: {max: 6.28}, speed: 2, rot: {z:'T*57.3 + 2.094'} } },
                        { type: 'parametric', name: 'Blade 3', config: { xExpr: '(1+u)*\\cos(v)', yExpr: '(1+u)*\\sin(v)', zExpr: '0.2*u*\\cos(v)', uMin: 0, uMax: 4, vMin: 0, vMax: 0.5, quality: 20 }, colormap: 'Jet', animation: { modes: {v:false, p:true}, T: {max: 6.28}, speed: 2, rot: {z:'T*57.3 - 2.094'} } }
                    ]
                };
            }
